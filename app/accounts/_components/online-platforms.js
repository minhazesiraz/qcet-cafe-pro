...
import { onlinePlatformsLogin } from "@/app/actions/accounts";

export default function OnlinePlatforms({ name }) {
   const onSubmit = async (providers) => {
      const formData = new FormData();
      formData.append("actions", providers);

      await onlinePlatformsLogin(formData);
   };
      ...
      <>
         <div className="flex flex-col gap-3 mb-6">
            ...
            <button
               ...
               className="flex items-center justify-center gap-2 border rounded-lg py-2 hover:bg-gray-100 cursor-pointer"
            >
               <svg
                  className="w-5 h-5"
